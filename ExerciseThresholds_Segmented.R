...
# assuming the data file and source file are in the working directory
df1 <- read_excel("Sample_Data_1.xlsx",sheet = "95")
#df1 <- read_excel("Sample_Data_2.xlsx",sheet = "38")
df1
...
gr2_get <- gr1_get + geom_line(data = model1, aes(x = VO2, y = VCO2), colour = "red",linewidth=1)+
  geom_vline(xintercept = seg_get$psi[,2], linetype = 2,colour="red",linewidth=1)
gr2_get
...
# plot the fitted model
gr2_rcp <- gr1_rcp + geom_line(data = model2, aes(x = VCO2, y = VE), colour = "red",linewidth=1)+
...
# plot the fitted model
gr2_rt <- gr1_rt + geom_line(data = model3, aes(x = watt, y = VE_PETCO2), colour = "red",linewidth=1)+
...
grid.arrange(gr2_get,gr2_rcp,gr2_rt,gr2_rt,ncol=2)
